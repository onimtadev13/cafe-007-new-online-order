...
class HomeScreen extends React.PureComponent {
  ...
  componentDidMount() {
    // Use separate variable names to avoid the bug where this._unsubscribe
    // gets overwritten by the blur listener
  ...
  render() {
    ...
    // ── Scroll-driven animations (all original, untouched) ────────────────────
    ...
    const EmptyListComponent = () => (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: '50%' }}>
        <Text style={{
          ...
          fontSize: 20,
          color: theme.text,     // ← was no colour (inherited)
        }}>
    ...
    return (
      <Animated.View style={[
        ...
        {/* ── Tab bar (fades in once scrolled) ── */}
        <Animated.View style={{ opacity: listOpacity }}>
          ...
          <Tabbar
            ...
            onPressClick={(index, headerindex) => {
              this.ref.scrollToIndex({ animated: false, index, viewOffset: -1 });
              this.setState({
        ...
        <Animated.View style={[
          ...
          <Animated.View style={[
            ...
            {this.state.Header !== null && this.state.Header !== '' ? (
              <>
                ...
                {/* Collapsed title — appears as header shrinks, uses theme colour */}
                <Animated.Text style={[
                  {
                    fontFamily: Platform.OS === 'ios' ? 'Asap-Regular_Bold' : 'AsapBold',
                    fontSize: 30,
                    position: 'absolute',
                    color: theme.text,  // ← was hardcoded 'black'
                  },
          ...
          <Animated.View style={[
            ...
            {/* Description always over the image — keep white */}
            <Text style={{
              width: (this.state.Header === null) | (this.state.Header === '') ? '50%' : '60%',
              color: 'white',
            }}>
          ...
          <Animated.View style={{ opacity: this.state.sliderUp }}>
            <Animated.FlatList
              ...
              onScroll={Animated.event(
                ...
                { useNativeDriver: true, listener: event => this.onScroll(event) },
              )}
  ...
  LoadHeaderImage() {
    fetch(APIURL, {
      ...
      body: JSON.stringify({
        HasReturnData: 'T',
        Parameters: [
          { Para_Data: '108', Para_Direction: 'Input', Para_Lenth: 10, Para_Name: '@Iid', Para_Type: 'int' },
        ],
...
function HomeScreenWrapper(props) {
  ...
  return <HomeScreen {...props} theme={theme} isDark={isDark} toggleTheme={toggleTheme} />;
}
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    // backgroundColor set dynamically via theme.bg in render
  },
  header: {
    position: 'absolute',
