...
class CheckoutScreen extends React.Component {
  ...
  getCouponIcon = coupon => {
    ...
    if (description.includes('spend') || description.includes('above'))
      return { name: 'basket-shopping', color: '#7a7a7a' };
    if (description.includes('weekend'))
      return { name: 'tag', color: '#7a7a7a' };
    if (description.includes('drink'))
  ...
  handleRemoveCoupon = () => {
    this.setState(
      { appliedCoupon: null, savedAmount: 0, promoCode: '', couponValue: '' },
      () => {
        this.GetTaxNetTotal(this.state.dineType);
      },
    );
    ...
    fetch(APIURL, {
      ...
      .then(json => {
        ...
        if (this.state.savedAmount > 0)
          calculatedNetTotal -= this.state.savedAmount;
        this.setState({
  ...
  GetLocation = () => {
    fetch(APIURL, {
      ...
      .then(res => res.json())
      .then(json => {
        const Locations = json.CommonResult.Table.map(element => ({
  ...
  ApplyCoupon = async () => {
    ...
    if (matchedCoupon) {
      ...
      if (typeof couponValue === 'string' && couponValue.includes('%')) {
        discountAmount =
          (subTotal * parseFloat(couponValue.replace('%', ''))) / 100;
      } else {
        discountAmount = parseFloat(couponValue);
      }
      this.setState(
    ...
    try {
      ...
      if (json.strRturnRes) {
        ...
        if (result.SUCESS === 'SUCESS') {
          ...
          if (
            ...
            discountAmount =
              (subTotal * parseFloat(apiCouponValue.replace('%', ''))) / 100;
          } else {
      ...
      showNetworkError(error, () => this.ApplyCoupon(), null);
    }
  };
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            >
              ...
              2. Some items in the order may be out of stock, and those items
              will not be delivered. Therefore, the total order value will be
              changed.
            </Text>
            <View
              style={{
            ...
            {this.state.discount !== 0 &&
              this.renderTotalRow('Discount', this.state.discount)}
            {this.state.dineType === 'Delivery' &&
              this.state.deliveryCharge !== 0 &&
              this.renderTotalRow('Delivery Charge', this.state.deliveryCharge)}
            {(this.state.dineType === 'PickUp' ||
              this.state.dineType === 'EatIn') &&
              this.state.serviceCharge !== 0 &&
              this.renderTotalRow('Service Charge', this.state.serviceCharge)}
            ...
            {this.state.subTotal !== this.state.netTotal &&
              this.renderTotalRow('Net Total', this.state.netTotal)}
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <Text
                          style={{
                            color: isHighlighted ? '#ffa363' : theme.textSub,
                            fontSize: 15,
                            fontWeight: 'bold',
                            marginLeft: 6,
                            fontFamily:
              ...
              {this.state.appliedCoupon && (
                <View
                  style={{
                    ...
                    padding: 12,
                    borderRadius: 10,
                    backgroundColor: isDark ? '#0d2a1a' : '#f2f9ff',
                    borderWidth: 1,
                    borderColor: isDark ? '#1c6638ff' : '#66b3ff',
                  }}
                >
                  <View style={{ flexDirection: 'column' }}>
                    <Text
                      style={{
                        color: isDark ? '#4fc97a' : '#0080ff',
                        fontWeight: 'bold',
                        fontSize: 15,
                      }}
                    >
            ...
            >
              ...
              <Card
                ...
                cardMaxElevation={12}
                cornerRadius={15}
                style={{ marginTop: 25, backgroundColor: theme.bg }}
              >
                <TouchableOpacity
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            <ScrollView>
              ...
              {this.state.tax !== 0 &&
                this.renderTotalRow('Tax', this.state.tax)}
              {this.state.discount !== 0 &&
                this.renderTotalRow('Discount', this.state.discount)}
              {this.state.dineType === 'Delivery' &&
                this.state.deliveryCharge !== 0 &&
                ...
                >
                  ...
                  <TextInput
                    style={{
                      ...
                      width: '100%',
                      marginBottom: 20,
                      color: this.state.isCustomPromo
                        ? theme.text
                        : theme.textMuted,
  ...
  async OnlineOrderDataSave() {
    ...
    fetch(PLACEORDERURL, {
      ...
      .catch(er => {
        this.touchableInactive = false;
        showNetworkError(er, () => this.OnlineOrderDataSave(), null);
      });
  }
  ...
  async OnlineOrderDataSaveBeforPay(isCard) {
    ...
    fetch(PLACEORDERURL, {
      ...
      .catch(er => {
        ...
        showNetworkError(
          er,
          () => this.OnlineOrderDataSaveBeforPay(isCard),
          null,
        );
      });
  }
