...
class ItemScreen extends React.PureComponent {
  ...
  LoadStoreStatus() {
    fetch(APIURL, {
      ...
      body: JSON.stringify({
        HasReturnData: 'T',
        Parameters: [
          {Para_Data: '112',                Para_Direction: 'Input', Para_Lenth: 10,  Para_Name: '@Iid',   Para_Type: 'int'},
          {Para_Data: this.state.LocationDB, Para_Direction: 'Input', Para_Lenth: 100, Para_Name: '@Text1', Para_Type: 'varchar'},
        ],
      ...
      .then(json => {
        ...
        if (json.CommonResult.Table[0].Shop_Status == 'T') {
          Alert.alert(
            ...
            [
              {text: 'Try Again', onPress: () => this.setState({isLoading: false})},
              {text: 'Close',     onPress: () => this.setState({isLoading: false})},
            ],
            {cancelable: false},
          );
        } else {
          if (this.state.Additionallist.length !== 0 || this.state.list.length !== 0) {
            if (!this.isScrolled && this.state.Location !== 'Cart' && this.state.scrollPosition <= 430) {
  ...
  LoadFoodDetails(ProductCode) {
    ...
    fetch(APIURL, {
      ...
      body: JSON.stringify({
        HasReturnData: 'T',
        Parameters: [
          {Para_Data: '111',       Para_Direction: 'Input', Para_Lenth: 10,  Para_Name: '@Iid',   Para_Type: 'int'},
          {Para_Data: ProductCode, Para_Direction: 'Input', Para_Lenth: 100, Para_Name: '@Text1', Para_Type: 'varchar'},
        ],
        SpName: 'sp_Android_Common_API', con: '1',
      }),
    })
      ...
      .catch(er => {
        console.log(er);
        Alert.alert(
          'Warning', "The operation couldn't be completed.",
          [
            {text: 'Try Again', onPress: () => this.LoadFoodDetails(this.state.ProductCode)},
            {text: 'Close'},
          ],
          {cancelable: false},
        );
      })
      .finally(() => {this.LoadFoodAddons(ProductCode);});
  }
  ...
  LoadFoodAddons(ProductCode) {
    fetch(APIURL, {
      ...
      body: JSON.stringify({
        HasReturnData: 'T',
        Parameters: [
          {Para_Data: '102',       Para_Direction: 'Input', Para_Lenth: 10,  Para_Name: '@Iid',   Para_Type: 'int'},
          {Para_Data: ProductCode, Para_Direction: 'Input', Para_Lenth: 100, Para_Name: '@Text1', Para_Type: 'varchar'},
        ],
        SpName: 'sp_Android_Common_API', con: '1',
      }),
        ...
        if (this.state.Location === 'Cart') {
          ...
          if (json.CommonResult.Table.length !== 0) {
            ...
            json.CommonResult.Table.forEach((element, index) => {
              list.push({id: index, addonsId: element.addonsId, name: element.name, price: parseInt(element.price), checked: false, color: 'black'});
            });
            if (PMax_Addons === addlist.length) {
              const unselectedList = list.filter(i => !i.checked);
              for (let i = 0; i < unselectedList.length; i++) { list[i].color = '#bababa'; }
          ...
          if (json.CommonResult.Table1.length !== 0) {
            PMax_AdditionalAddons = json.CommonResult.Table1[0].product_max_extras;
            json.CommonResult.Table1.forEach((element, index) => {
              additionallist.push({id: index, extrasId: element.extrasId, name: element.name, price: parseInt(element.price), checked: false, color: 'black'});
            });
            if (PMax_AdditionalAddons === extralist.length) {
              const unselectedAdditional = additionallist.filter(i => !i.checked);
              for (let y = 0; y < unselectedAdditional.length; y++) { additionallist[y].color = '#bababa'; }
              for (let a = 0; a < extralist.length; a++) {
                const idx = additionallist.findIndex(x => x.name === extralist[a].name);
                additionallist[idx].checked = true;
                additionallist[idx].color = 'black';
              }
            } else {
              for (let z = 0; z < extralist.length; z++) {
                const idx = additionallist.findIndex(x => x.name == extralist[z].name);
                additionallist[idx].checked = true;
                additionallist[idx].color = 'black';
              }
            }
          ...
          this.setState({
            ...
            list, Additionallist: additionallist, RequiredItemList,
            isLoading: false,
          ...
          this.setState({
            list, Additionallist, RequiredItemList,
            Max_Addons: PMax_Addons,
            Max_Additional_Addons: PMax_AdditionalAddons,
            isLoading: false,
          });
      ...
      .catch(error => {
        ...
        Alert.alert(
          'Warning', "The operation couldn't be completed.",
          [
            {text: 'Try Again', onPress: () => this.LoadFoodAddons(this.state.ProductCode)},
            {text: 'Close'},
          ],
          {cancelable: false},
        );
  ...
  renderAdditionalAddons() {
    ...
    return this.state.Additionallist.map((item, key) => (
      <TouchableOpacity key={key} onPress={() => this.checkThisAdditionalBox(item.id)}>
        <View style={{
          ...
          <CheckBox
            ...
            tintColors={{true: theme.accent, false: item.color === 'black' ? theme.text : '#bababa'}}
            onAnimationType={'fill'}
            value={item.checked}
          />
          <Text style={{
            flex: 1.2, fontSize: 18,
          ...
          <NumericFormat
            ...
            renderText={formattedValue => (
              <Text style={{
                flex: 0.8, textAlign: 'right', marginRight: 10, fontSize: 18,
                fontFamily: Platform.OS === 'ios' ? 'Asap-Regular' : 'AsapRegular',
                color: item.color === 'black' ? theme.text : theme.textMuted,  
              }}>+ {formattedValue}</Text>
            )}
  ...
  checkThisAdditionalBox(itemID) {
    ...
    if (this.state.Max_Additional_Addons === 0) {
      ...
      if (additionallist[itemID].checked) {
        this.setState({Amount: this.state.Amount + additionallist[itemID].price});
      } else {
        this.setState({Amount: this.state.Amount - additionallist[itemID].price});
      }
    } else {
      if (checked.length === this.state.Max_Additional_Addons) {
        if (additionallist[itemID].checked) {
          additionallist[index].checked = !additionallist[index].checked;
          this.setState({Amount: this.state.Amount - additionallist[itemID].price});
          for (let i = 0; i < unchecked.length; i++) { unchecked[i].color = 'black'; }
        }
      } else {
        additionallist[index].checked = !additionallist[index].checked;
        ...
        if (NewChecked.length === this.state.Max_Additional_Addons) {
          for (let i = 0; i < NewUnChecked.length; i++) { NewUnChecked[i].color = '#bababa'; }
        }
        if (additionallist[itemID].checked) {
          this.setState({Amount: this.state.Amount + additionallist[itemID].price});
        } else {
  ...
  checkThisRadio(item) {
    item.forEach(element => {
      if (element.selected) { this.setState({SelectedRequiredItem: element.value}); }
    });
  }
  ...
  onAddtoCart = () => {
    if (!this.touchableInactive) {
      ...
      for (let i = 0; i < checked.length; i++) {
        if (checked[i]) {
          selected.push({
            OrderID: this.state.OrderID, CartItemID,
            ProductCode: this.state.ProductCode,
            AddonsCode: itemcode[i], name: item[i], price: itemprice[i],
          });
        }
      }
      ...
      for (let y = 0; y < additional_checked.length; y++) {
        if (additional_checked[y]) {
          Extralist.push({
            OrderID: this.state.OrderID, CartItemID,
            ProductCode: this.state.ProductCode,
            ExtraCode: itemcode2[y], name: item2[y], price: itemprice2[y],
          });
        }
      }
      ...
      if (this.state.Location === 'Cart') {
        ...
        let Product = {
          ...
          Addons: selected, Extra: Extralist,
        ...
        if (this.state.userlog !== null) {
          if (this.props.cartItems.length === 0) { this.onClearAsync(); }
          AsyncStorage.setItem(CartItemID, JSON.stringify(Product));
        }
  ...
  onEnableLoader = () => {
    if (this.state.Shop_Status == 'T') {
      ...
      if (
        (this.state.Additionallist.length === 0 && this.state.list.length === 0) ||
        this.state.Location === 'Cart'
      ) {
        this.setState({isLoading: true});
      } else {
        if (this.isScrolled) { this.setState({isLoading: true}); }
      }
    }
  };
      ...
      <View style={{flex: 1, backgroundColor: theme.bg}}>  
        ...
        <Animated.ScrollView
          ...
          onScroll={Animated.event(
            [{nativeEvent: {contentOffset: {y: this.state.scrollY}}}],
            {
              ...
              listener: e => this.setState({scrollPosition: e.nativeEvent.contentOffset.y}),
            },
          )}
        >
          <View style={{marginTop: 20, marginLeft: 20, marginRight: 20, paddingTop: 20}}>
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <View style={{flex: 1}}>
                ...
                <Text style={{
                  textTransform: 'lowercase', fontSize: 16,
                  fontFamily: Platform.OS === 'ios' ? 'Asap-Regular' : 'AsapRegular',
                  color: theme.textSub,        
                }}>@{this.state.ProductName}</Text>
              </View>
              ...
              {this.state.isPopular && (
                <View style={{marginRight: 20, alignItems: 'center', marginTop: 20}}>
                  <View style={{
                    backgroundColor: '#FF6900',  // keep brand orange
                    borderRadius: 10,
          ...
          <View style={{alignItems: 'center', marginTop: 20}}>
            ...
            {this.state.isDiscounted && (
              <Text style={{
                color: '#FF5722',              // keep brand discount colour
          ...
          {this.state.Location === 'Cart' ? (
            <TouchableOpacity
              style={{width: 190, alignSelf: 'center', marginTop: 20, margin: 10}}
              onPressIn={() => {this.setState({isLoading: true});}}
              onPress={() => this.onRemoveFromCart()}
            >
              <Text style={{
                textAlign: 'center', color: 'red',  // keep red — danger action
                fontSize: 18,
                fontFamily: Platform.OS === 'ios' ? 'Asap-Regular' : 'AsapRegular',
        ...
        <View style={{
          ...
          <View style={{flex: 1, flexDirection: 'row', left: 20}}>
            ...
            <Text style={{
              margin: 10, fontSize: 22, width: '18%', textAlign: 'center',
              fontFamily: Platform.OS === 'ios' ? 'Asap-Regular' : 'AsapRegular',
              color: theme.pillText,         
            }}>{this.state.itemQty}</Text>
        ...
        <Animated.View style={[
          ...
          <Animated.View style={[
            ...
            <FastImage
              style={[styles.headerImage, {backgroundColor: 'rgba(0,0,0,0.4)'}]}
              source={
                this.state.ProductIMG === ''
                  ? require('../assets/image-placeholder.png')
                  : {uri: this.state.ProductIMG, priority: FastImage.priority.normal}
              }
  ...
  <Animated.View style={[
    {
      ...
      backgroundColor: '#FFFFFF',    // ← always white, same as light theme
      alignItems: 'center', justifyContent: 'center',
    },
    ...
    />
        ...
        {/* ── ThemeToggle (absolute top-right, same level as back button) ── */}
        <View style={{
          position: 'absolute',
          top: 20,
          right: 16,
        ...
        {this.state.isLoading ? (
          <View style={{
            ...
            alignItems: 'center', justifyContent: 'center',
            backgroundColor: 'rgba(0,0,0,0.7)',  // keep dark overlay always
          }}>
            <View style={{
              width: 80, height: 80,
              ...
              <ActivityIndicator size={30} color={theme.textMuted} animating={true} />
              <Text style={{
...
const mapDispatchToProps = dispatch => ({
  ...
  updateItemToCart:   product => dispatch({type: 'UPDATE_FROM_CART', payload: product}),
...
export default connect(mapStateToProps, mapDispatchToProps)(withTheme(ItemScreen));
